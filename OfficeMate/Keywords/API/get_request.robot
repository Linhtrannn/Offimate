...
Get And Vaidation
    ...
    ${status}       ${resp}=    BuiltIn.Run Keyword And Ignore Error    api_request.Get Request     ${is_flash_deal}          ${flash_deal_enable}    ${sort}                 ${type_id}         ${typeid}        ${flash_deal_from}
    ...            ${flash_deal_from_day}    ${flash_deal_to}        ${flash_deal_to_day}    ${x_subject_id}    ${visibility}
    ...            ${status}                 ${cookie}               ${x-newrelic-id}        ${x-store-code}    ${api_url}
    
    Run Keyword If    '${status}'=='PASS'    Status Should Be    ${response_code}    ${resp}
...
Get Reponse Payload
    ...
    ${resp}=    api_request.Get Request     ${is_flash_deal}          ${flash_deal_enable}    ${sort}                 ${type_id}         ${typeid}        ${flash_deal_from}
    ...            ${flash_deal_from_day}    ${flash_deal_to}        ${flash_deal_to_day}    ${x_subject_id}    ${visibility}
    ...            ${status}                 ${cookie}               ${x-newrelic-id}        ${x-store-code}    ${api_url}
